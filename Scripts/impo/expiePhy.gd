...
# also tried doing this with a match statement but this was easier
var isWalking = false
...
func pair(boneName: String, body: RigidBody2D):
	...
	if boneNode == null:
		#print("couldnt find bone: ", boneName)
		return
...
func TweenValue(start: float, end: float, time: float):
	var tween := create_tween()
	tween.tween_method(updateVal, start, end, time)

func updateVal(new_val: float):
	blendValue = new_val
